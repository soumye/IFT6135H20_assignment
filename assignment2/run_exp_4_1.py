...
parser.add_argument('--file', type=str, help='file to load the weights')
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    #Load the Model
    ckpt = torch.load(args.file, map_location=device)
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        #Modified Code to compute the loss(batch_averaged) for only the last time step.
        # tt = torch.squeeze(targets.view(-1, model.batch_size * model.seq_len))
        tt_last = torch.squeeze(targets[-1].view(-1, model.batch_size))
        ...
        loss = loss_fn(outputs[-1].contiguous().view(-1, model.vocab_size), tt_last)
        ...
        if is_train:  # Only update parameters if training
            ...
            l = []
            for i in range(35):
                l.append(torch.norm(hidden_list[i][1].grad).data.item())
            print(l)
